...
class MappingTable extends Component {
  ...
  CustomRow = ({row}) => {
    ...
    return (
      ...
      >
        ...
        <td>
          {format === 'string' && (
            <TextIcon fill="#fdd835"/>
          )}
          {format === 'number' && (
            <NumberIcon fill="#fdd835"/>
          )}
          {format === 'boolean' && (
            <BoolIcon fill="#fdd835"/>
          )}
          {format === 'object' && (
            <JsonIcon fill="#fdd835"/>
          )}
          {format === 'email' && (
            <EmailIcon fill="#fdd835"/>
          )}
          {format === 'date' && (
            <DateIcon fill="#fdd835"/>
          )}
          {format === 'xml' && (
            <XmlIcon fill="#fdd835"/>
          )}
  ...
  render() {
    ...
    const props = {
      title: 'Field Mapping',
      subtitle: "Configure the Relationship between CSV Fields and Content type Fields",
